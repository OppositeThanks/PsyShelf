...
(async()=>{
 ...
 await page.evaluate(()=>window.psyLibrary.addFiles({storageMode:'reference'}));await page.evaluate(()=>loadResources());
 await page.locator('#searchInput').fill('violet lantern');await page.locator('#documentSearchForm summary').click();await page.locator('#documentSearchOCR').check();await page.locator('#documentSearchStart').click();
 ...
 const opened=app.waitForEvent('window');await page.locator('.document-hit button').click();const preview=await opened;await preview.waitForFunction(()=>document.querySelector('#pdfPage')?.value==='1');await preview.close();
 for(const [language,expected] of [['French','Appliquer les options de recherche'],['Spanish','Aplicar opciones de búsqueda']]){
 await page.locator('#settingsButton').click();await page.locator('#interfaceLanguage').selectOption(language);await page.waitForFunction(l=>window.psyI18n.language===l,language);await page.locator('[data-close="settingsDialog"]').click();assert.equal(await page.locator('#documentSearchStart').textContent(),expected);assert.match(await page.locator('.document-hit blockquote').textContent(),/violet lantern/);
 }
