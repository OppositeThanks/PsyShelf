...
Keep user-facing replies short. Expand explanations only when asked. Minimize unnecessary tool output and repeated checks while still validating changes.

For every requested modification, update README.md in the same commit to describe the changes. Keep the relevant feature, setup, and usage instructions current, and record a concise dated summary in its update history. Document completed behavior accurately and distinguish unpublished changes from available releases.
